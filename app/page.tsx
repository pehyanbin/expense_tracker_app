...
export default function ExpenseTracker() {
  ...
  return (
    <div className={`${darkMode ? "dark" : ""}`}>
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 md:p-8 font-sans text-gray-800 dark:text-gray-100 transition-colors duration-300">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <header className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 bg-white dark:bg-gray-900 p-2 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800">
              {/* Theme Toggle */}
              <button 
                onClick={toggleTheme}
              ...
              <div className="flex items-center gap-2 border-l border-gray-200 dark:border-gray-700 pl-3">
                <button onClick={() => navigateTime("prev")} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition-colors">
                  <ChevronLeft className="w-5 h-5 text-gray-600 dark:text-gray-400" />
                </button>
                <span className="min-w-[140px] text-center font-bold text-gray-800 dark:text-gray-200 select-none">{timeLabel}</span>
                <button onClick={() => navigateTime("next")} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 space-y-6">
              <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 transition-colors">
                <h2 className="text-xl font-semibold mb-4 flex items-center gap-2 text-gray-800 dark:text-gray-100">
                  {editingId ? <Edit2 className="w-5 h-5 text-orange-500"/> : <PlusCircle className="w-5 h-5 text-indigo-500 dark:text-indigo-400"/>}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none dark:text-white transition-colors"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full p-2 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-white dark:bg-gray-800 dark:text-white transition-colors"
                      >
                  ...
                  <div className="flex gap-2 pt-2">
                    <button type="submit" className={`flex-1 py-2 rounded-lg text-white font-medium ${editingId ? "bg-orange-500" : "bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-700 dark:hover:bg-indigo-600"}`}>
                      {editingId ? "Update" : "Add"}
                    </button>
                    {editingId && <button type="button" onClick={cancelEdit} className="px-4 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg">Cancel</button>}
                  </div>
                </form>
              </div>

              {/* Transaction List */}
              <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col h-[400px] transition-colors">
                <div className="flex justify-between items-center mb-2">
                  <h2 className="text-xl font-semibold flex items-center gap-2 text-gray-800 dark:text-gray-100">
                    History
                    {selectedDay && (
                       <span className="text-sm font-normal bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300 px-2 py-0.5 rounded-full">
                ...
                <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-3">
                  {displayList.length === 0 ? (
                    ...
                    [...displayList].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((t) => (
                      <div key={t.id} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl hover:bg-indigo-50 dark:hover:bg-gray-800 group border border-transparent dark:border-gray-800 transition-colors">
            ...
            <div className="lg:col-span-2">
              <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 h-full min-h-[500px] transition-colors">
                <div className="flex justify-between items-center mb-6">
                   <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-100">{viewMode === 'daily' ? 'Monthly Calendar' : 'Yearly Overview'}</h2>
                </div>
                ...
                {viewMode === "daily" ? (
                        ...
                        return (
                          <button 
                            ...
                            className={`
                              ...
                              ${isSelected 
                                  ? 'border-indigo-600 dark:border-indigo-500 ring-2 ring-indigo-200 dark:ring-indigo-900 bg-indigo-50 dark:bg-indigo-900/20 z-10' 
                                  : 'border-gray-100 dark:border-gray-800 hover:border-indigo-300 dark:hover:border-gray-600 bg-white dark:bg-gray-900'
                              }
                            `}
                          >
                            ...
                            <div className="space-y-1 w-full">
                              {cell.income > 0 && (
                                 <div className="text-[10px] bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 px-1.5 py-0.5 rounded-md font-medium truncate">
                                   +{formatCurrency(cell.income)}
                                 </div>
                              )}
                              {cell.expense > 0 && (
                                 <div className="text-[10px] bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400 px-1.5 py-0.5 rounded-md font-medium truncate">
                  ...
                  <ResponsiveContainer width="100%" height={400}>
                    <BarChart data={monthlyChartData}>
                      ...
                      <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: darkMode ? '#9ca3af' : '#9ca3af'}} />
                      <YAxis axisLine={false} tickLine={false} tick={{fill: darkMode ? '#9ca3af' : '#9ca3af'}} />
                      <Tooltip 
                         contentStyle={{
                             borderRadius: '8px', 
                             border: 'none', 
